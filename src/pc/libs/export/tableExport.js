...
(function(view){
    ...
    var getText = function( el ){
        ...
        var v = s == null ? "" : s.replace( /^\s*(.*?)\s+$/, "$1");
        if(v>9007199254740992){
            v = v+'&nbsp;';//数字过大的拼接一个字符，让其原样显示
        }
        return v;
    };
    view.tableExport = function(tableId, filename, type, excelData){
        ...
        var toExcel = function(excelData){
            let data = excelData;
            var tmpl = '<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:'+type+'" xmlns="http://www.w3.org/TR/REC-html40">';
            tmpl += '<head><meta charset="'+charSet+'" /><!--[if gte mso 9]><xml><x:ExcelWorkbook><x:ExcelWorksheets><x:ExcelWorksheet><x:Name>';
            tmpl += '{worksheet}</x:Name><x:WorksheetOptions><x:DisplayGridlines/></x:WorksheetOptions></x:ExcelWorksheet></x:ExcelWorksheets></x:ExcelWorkbook></xml><![endif]-->';
            ...
            for(var j =0, col; col = table.rows[0].cells[j]; j++){
                //office += mapb[0][0]+ getText(col) +mapb[0][1];
                office += '<th style="width: '+ col.clientWidth+ '">' + getText(col) +mapb[0][1];
            }
            office += maph[0][1];
            /* tbody start*/
            office += maph[1][0];
            for (var i=0; i<data.length; i++) {
                ...
                for (var attrs in data[i]) {
                    //console.log(data[i][attrs]);
                    office += mapb[1][0]+ data[i][attrs] +mapb[1][1];
                }
                office += maph[2][1];
            }
            office += maph[1][1];
        ...
        var typeMap = {
            ...
            doc: toOffice,
            docx: toOffice,
            xls: toExcel, // toOffice,
            xlsx: toOffice
        };

        typeMap[type](excelData);
    };
